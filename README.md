...
**Auto Equipment** prüft alle fünf Sekunden bezahlbare Verbesserungen für Angeln und Messer. Angeln werden zuerst nach Stärke, dann Traggewicht, Glück und Geschwindigkeit verglichen; Messer nach Wertbonus, kritischer Trefferchance und Schnittfenster. Die beste bezahlbare Verbesserung der Angel wird zuerst gekauft, danach wird das verbleibende Geld für das Messer geprüft. Nur Shop-Angeln und die sechs regulären Cash-Messer werden gekauft. Bereits vorhandene bessere Spezialausrüstung bleibt berücksichtigt. Besitz und Ausrüstung müssen bestätigt werden; abgelehnte Käufe schalten nur Auto Equipment ab. Dieser Schalter startet ebenfalls ausgeschaltet und wird beim Entladen beendet.

Ab 2.10.0 rüstet LUB vor dem Servieren den passenden Teller aus. Es sucht nach der Teller-ID im Charakter und Rucksack; eine eindeutige Zuordnung über Rezept und Fisch dient als Fallback ohne ID. Physische Teller im eigenen Restaurant mit passender ID und Rezeptname können über die aufgezeichnete EquipPlate-Interaktion aufgenommen werden. Fehlende oder mehrdeutige Teller führen zu einer konkreten Fehlermeldung. Servieren und Ausrüstungswechsel warten auf das Ende eines laufenden Fangs und verhindern bis zu ihrer Bestätigung einen neuen Wurf, damit die Angel den Teller nicht wieder ablegt. Diese Änderungen wurden lokal mit simulierten Tools und Serverantworten geprüft; die Tellerattribute und Kaufbestätigung im Live-Spiel sind noch nicht bestätigt.
...
Fische, Zutaten und fertige Gerichte werden aus den aktuellen Spieldaten gewählt. Teichfische und Favoriten werden nicht verwendet. Als zusätzlicher Favoritenschutz pausiert der Autofarm eine Rezeptart, sobald ein entsprechendes favorisiertes Gericht vorhanden ist. Vor dem Verbrauch eines Fisches und vor dem Kochen wird die Kundenbestellung erneut geprüft; verschwundene oder geänderte Bestellungen werden verworfen. Fremde Kunden werden nicht bedient.
...
Ab 2.16.0 enthält LUB Pop Bubbles mit sechs unabhängig zuschaltbaren, zunächst ausgeschalteten Modi:

- **Autofarm** verwendet `BubbleRenderer:getClosestBubbleModelToPosition`, das bereits getroffene, verschwindende und durch Trefferprognosen ausgeblendete Bubbles ausschließt. Ziel ist die aktuelle PrimaryPart-Position. Solange Autofarm aktiv ist, verwendet auch ein bereits laufender automatischer Wurfcontroller diese Zielauswahl, sodass seine Feuerrate nicht für andere Ziele verbraucht wird. Auch bei gedrückter Maustaste wird vor jedem Wurf die nächste gültige Bubble zur aktuellen Spielerposition gewählt. Beim Abschalten, Entladen oder Fehler wird die vorherige Zielmethode wiederhergestellt. Der WeaponController erzeugt die lokale Waffe und bedient die Treffer-Callbacks. Der AutoAttack-Schalter bleibt unverändert. Der normale Wurf dreht die Figur zum Ziel. **Throw delay** ist eine zusätzliche Begrenzung (Standard 0); die maximale Feuerrate bestimmt der WeaponController.
- Cash-, Gem- und Essence-Drops (Flammen) werden während Autofarm anhand der aktuellen Spawn-Meldungen eingesammelt. Entfernte oder abgelaufene Drops werden verworfen; unbestätigte Sammlungen werden frühestens nach zwei Sekunden erneut angefordert, höchstens 20 IDs pro Paket. Vorhandene Flammen werden zusätzlich anhand der Modelle `EssenceDrop_<ID>` im eigenen `LocalEssenceDrops_<UserId>`-Ordner erkannt. Bereits bestätigte Sammelanimationen werden nicht erneut als Drops aufgenommen. Vor dem Laden gemeldete Cash- und Gem-Drops sind erst nach einer neuen Spawn-Meldung bekannt.
- **Auto Equip Best Bubblets** prüft alle 30 Sekunden mit der Schadensformel des Spiels, ob ein Inventar-Bubblet stärker als der schwächste ausgerüstete Bubblet ist oder einen freien Slot verbessern kann. Nur dann wird `BubbletEquipBestRequest` aufgerufen. Level, Mutation und Modifier fließen in den Vergleich ein; Gleichstand löst keinen Aufruf aus. Die endgültige Auswahl übernimmt das Spiel.
...
**Auto Bubblet Boost** erneuert den Boost der eigenen Basis automatisch nach Ablauf. Ein aktiver Boost wird nicht erneut angefordert. Der Server verlangt Nähe zur eigenen Basis; bei einer Ablehnung wird nach fünf Sekunden erneut versucht. LUB bewegt die Figur dafür nicht.

**Auto Upgrade Bubblets** fordert mit 0,03 Sekunden Pause nach den Serverantworten für jeden unter **Equipped Bubblet slots** eingestellten Slot (Standard sechs, Slot-Indizes 0–5; auf die eigene Anzahl einstellen) gleichzeitig jeweils einen Level-Schritt an. Pro Slot läuft höchstens eine Anfrage; erst nach allen Antworten beginnt die nächste Runde. Damit addieren sich die Antwortzeiten verschiedener Slots nicht mehr. Bei Geldmangel wird am noch unbezahlten Slot fortgesetzt, statt erneut den ersten Slot zu bevorzugen. Vollständig gelevelte Slots werden übersprungen. **Auto Rebirth** sammelt auch ohne Autofarm bekannte Flammen und fordert alle fünf Sekunden einen Rebirth an; nicht erfüllte Voraussetzungen werden erneut geprüft, nur eine positive Serverantwort wird als Erfolg angezeigt. Beide Schalter starten ausgeschaltet. Normale Upgrades und Rebirth laufen unabhängig; ausgegebenes Geld kann daher die nächste Rebirth-Bedingung verzögern.
...
Mit Dateizugriff werden die Einstellungen unter `LUB/Config.json` gespeichert. Chicken Farm und Sell Ores haben getrennte gespeicherte Einstellungen; Fishing Chef wird pro Sitzung gestartet; alte Einträge für nicht unterstützte Spiele werden entfernt. Ohne Dateizugriff gelten die Einstellungen für die Sitzung. Das erneute Ausführen öffnet ein bereits laufendes LUB 2.16.0; eine noch laufende Oberfläche der vorherigen LUB-Version wird beim Upgrade beendet und ersetzt.
...
Version 2.13.0 korrigiert den Wurfablauf anhand des bereitgestellten WeaponController: Das bisherige direkte ThrowWeapon mit true meldete fälschlich eine bereits lokal erzeugte Waffe. Der Controller-Aufruf erzeugt diese tatsächlich und beachtet seine aktuelle Feuerrate. Die Integration und Flammen-Sammlung wurden lokal geprüft; Live-Treffer sind noch nicht bestätigt.

Version 2.13.2 behebt den durch Live-Diagnosedaten nachgewiesenen Zielauswahlfehler: Die Outer-Teile der Bubbles sind transparent und wurden bisher vollständig ausgeschlossen. Ein Regressionstest deckt ausschließlich transparente Outer-Ziele, Vorrang vor Dekoration und Zielwechsel ab.

Der Diagnose-Export bereinigt ab dieser Version auch ungültiges UTF-8 und nicht endliche Zahlen im vollständigen Bericht. Falls JSONEncode weiterhin scheitert, wird eine .txt-Datei mit den erfassten Daten gespeichert und kopiert. Die Exporttests decken beide Fälle ab.
